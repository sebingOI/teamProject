...
const charr = [{ nick: kk, Lx: 453.2335, Ly: 441.21, Lz: 929.193, Rz: 50, cla: 1, pmHP: 100, pcHP: 100, lv: 1, pExp:9, act: 01 },
    { nick: pd, Lx: 453.1232, Ly: 440.21, Lz: 920.193, Rz: 50, cla: 2, pmHP: 100, pcHP: 100, lv: 1, pExp:9, act: 01 },
    { nick: jj, Lx: 490.1232, Ly: 442.21, Lz: 933.193, Rz: 50, cla: 3, pmHP: 100, pcHP: 100, lv: 1, pExp:9, act: 01 }];
socket.bind(9000);
...
function Udb(cmd, nick, Lx, Ly, Lz, Ry, cla, lv, pExp, act, Ipaddress, port) {
    this.cmd = cmd;
    this.nick = nick;
    this.Lx = Lx;
    this.Ly = Ly;
...
function CLS(cmd, nick, Lx, Ly, Lz, Ry, cla, lv, pExp, pAct, Ipaddress, port) {
    this.cmd = cmd;
    this.nick = nick;
    this.Lx = Lx;
    this.Ly = Ly;
...
socket.on('message', function(msg, rinfo) {
    ...
    if (str.cmd == "CS") {
        try {
            const csid = User.findOne({
                attributes: ['id', 'nick', 'Lx', 'Ly', 'Lz', 'Ry', 'cla', 'lv', 'pExp', 'pAct'],
                where: { id: str.id }
            });
            //console.log(csid);
            const message = JSON.stringify({cmd: "uDB", nick: csid.nick, Lx: csid.Lx, Ly: csid.Ly, Lz: csid.Lz, Ry: csid.Ry, cla: csid.cla, lv: csid.lv, pExp: csid.pExp, pAct: csid.pAct })
            console.log(message);
            let udb = new Udb("uDB", csid.nick, csid.Lx, csid.Ly, csid.Lz, csid.Ry, csid.cla, csid.lv, csid.pExp, csid.act, rinfo.Ipaddress, rinfo.port);
    ...
    if (cnt == 1000) {
     ...
     if (str.cmd == "DBs") {
        try {
            // 배열 전체 보내야함
            const message = JSON.stringify({cmd: "cLs", charr: charr })
            let cls = new CLS("cLs", charr, rinfo.Ipaddress, rinfo.port);
            socket.send(message, 0, message.length, cls.port, 
                function(err) {
                    if (err) {
                        console.log('메세지 전송 실패');
                        return;
